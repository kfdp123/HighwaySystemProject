    ...
    $.ajax({
        ...
        success: function (msg) {
            ...
            for (var i = 0; i < ilen; i++) {
                ...
                vjson_carlist[i].text = vjson_carlist[i].LICENSEPLATE;
            }
...
function make_detail_car(vattcar)
{
		...
		vdet+= ("登记时间:" + vattcar.CAR_REG_DATE + "</br>");
		vdet+= ("车辆编号:" + vattcar.ENGINE_ID + "</br>");
		vdet+= ("最近一次定位时间:" + vattcar.LAST_TIME + "</br>");
...
{
			var vdet="<p>";
		var vlen=vatt.length;
		for (var i=0;i<vlen;i++)
		{
			vdet += (vgcolums[i] + ":" + vatt[i] + "</br>");
		}
		vdet += "</p>"
...
function create_marker_car(vlon, vlat, vatt, vimg, vtt) {
		var vid=vgcar.values.length;
		var venet = create_marker([vlon,vlat],vid,vatt.ENGINE_ID,make_detail_car(vatt),vimg,32,32);
		vgcar.add(venet);
}
...
function fill_markers_person() {
    ...
    {
    	var vid=vgper.values.length;
			var venet = create_marker([vx,vy],vid, vjson_perlist[i].NAME,
...
function fill_markers() {
    ...
    for (var i = 0; i < vjson_carlist.length; i++) {
        ...
        vx = parseFloat(vijkj.X),
        vy = parseFloat(vijkj.Y);
        // 对于无效的数字给个初始值；而不能忽略之
        if (vx <= 60 || vy <= 10 || isNaN(vx) || isNaN(vy)) {
            vx = INITX; vy = INITY;
        }
		var vid=vgcar.values.length;
		var venet = create_marker([vx, vy],vid,vijkj.ENGINE_ID,
			make_detail_car(vijkj),'images/car32.png',32,32);
		vgcar.add(venet);
		
        create_marker_car(vx, vy, vijkj, 'images/car32.png');
...
function stat_track(msg, vqtp) {
    ...
    if (vquetype < 200) {
        ...
        if (vgcar != null) vgcar.clear();
        update_track_line(msg, vjson_carlist[vsel_car_idx].DRIVER_NAME,
...
function OnChangewarn(value) {
    ...
    else if (vcp === 'per')//人员只需要判断电子围栏即可以及是否出勤
    {
        ...
        vg0 = vgper[value];// vgper.graphics[value];
    }
}
...
function update_track_line(linepts, vvname, vimg, vtp) {
    ...
    for(var i=0;i<vlen;i++)
    {
        ...
        if(i==0||i==(vlen-1))//仅首尾有标签
        {
	        ...
	        var vpic = (i == 0 || i == (vlen - 1)) ? vimg : '';
			    ...
			    if (vtp === 'car') {
			    		vht = create_marker(vx, vy, vijklmn, vpic, vvname);
		        }
		      if (vtp === 'per') {
			      	...
			      	vht = create_marker_person(vppx, vppy, vijklmn, vpic, vvname);
		      }    
		      if(i==0)head=vht;
		      else tail=vht;	
        }
	...
	//polyline
	var vpl=create_polyline(vposs,linepts[0].ID,'');
...
function getdist(vpt1, vpt2) {
    //var vdddd = esri.geometry.getLength(vpt1,vpt2);
    var vdx = vpt2.x - vpt1.x;
    var vdy = vpt2.y - vpt1.y;
...
function rt_pos(vpara) {
    ...
    if (vtimer === null) vtimer = setInterval(vcbfun, 5000);
}
...
function timer_rt_pos(vpara) {
    ...
    $.ajax({
        ...
        error: function (datas) {
            //请求出错处理
            $.messager.alert('timer_rt_pos()', err.message, 'info');
        }
    });
}
...
function update_marker_per(vijklmn) {
    var vidx = find_per_idx(vijklmn.PCID);
    if (-1 === vidx) return null;
    ...
    vgper[vidx]= create_marker([vx,vy],vijklmn.ID,vijklmn.PCID,make_detail(vijklmn),
    	'../images/per32.png',32,32);
}

function update_marker_car(vijklmn) {
    var vidx = find_car_idx(vijklmn.PCID);
    if (-1 === vidx) return null;

    var vx = parseFloat(vijklmn.X);
    var vy = parseFloat(vijklmn.Y);
    
    vgcar[vidx]=create_marker([vx,vy],vijklmn.ID,vgi.DRIVER_NAME,make_detail(vijklmn),
    	'../images/car32.png',32,32);

    if (vsped >= vspeed_max) {
        vttl = '车辆超速行驶！';
        vmsssg = "驾驶员：" + vgi.RIVER_NAME +
                "<br/>当前速度： " + vsped +
...
function update_markers(vrtpos) {
    ...
    //if (isinited(vpt)) viewer.flyto(vpt);
    viewer.entities.removeAll();
    viewer.entities  = (vtp === 'car') ? vgcar : vpger;
    viewer.zoomTo(viewer.entities);
...
function submitForm() {
    ...
    $.ajax({
        ...
        data: {
            'QTYPE': vqkey,
            'ID': vvid,
            'FROM': $("#tqbeginstat").textbox('getValue'),
            'TO': $("#tqendstat").textbox('getValue')
        },
        success: function (msg) {
