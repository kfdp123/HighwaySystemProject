     var drawFlag_m2=false;
	 //var scene1=viewer.scene;
	 //var ellipsoid1=scene.globe.ellipsoid;
     var cartesian_m2 = null;
	 var polygonPath_m2=[];//存储面的坐标值
	 var faceToline_m=[];//存储第一和第二个点的坐标值
	 var polyline_m1 = undefined;//第一个点和第二点连线
	 var polygon_m=undefined;
     var flagfaceline_m=0; 
	 var hander_m2=new Cesium.ScreenSpaceEventHandler(scene.canvas);
	//按钮事件
	...
	radio2.addEventListener("click", function(){
		...
		//hander2.removeInputAction(Cesium.ScreenSpaceEventType.RIGHT_CLICK);
			...
			drawFlag_m2=true;
		  flagfaceline_m=flagfaceline_m+1;
	//鼠标左键单击事件
	hander_m2.setInputAction(function(event){
		cartesian_m2 = viewer.scene.pickPosition(event.position);
		if(cartesian_m2){
		    if(drawFlag_m2){
			    polygonPath_m2.push(cartesian_m2);
				if(faceToline_m.length==0){
				faceToline_m.push(cartesian_m2);
				}
			}
		}
	...
	//鼠标移动事件
	hander_m2.setInputAction(function(event){
	 cartesian_m2 = viewer.scene.pickPosition(event.endPosition);
	   if(drawFlag_m2==true){
	      if(cartesian_m2){
		     if (polygonPath_m2.length < 2) {
                    if((faceToline_m.length==1||2)&&(polygonPath_m2.length==1)){
				    
                   if (!Cesium.defined(polyline_m1)) {
                    faceToline_m.push(cartesian_m2);
                    polyline_m1 = new CreatePolyline11(faceToline_m);
                    } else {
					
                    polyline_m1.path.pop();
                    polyline_m1.path.push(cartesian_m2);
                    }					 
					}
					return;
                }
			 if (!Cesium.defined(polygon_m)) {
				 viewer.entities.removeById('faceline'+flagfaceline_m);
                    polygonPath_m2.push(cartesian_m2);
                    polygon_m = new CreatPolygon_m1(polygonPath_m2);               
		        }else {
                    polygon_m.path.pop();
                    polygon_m.path.push(cartesian_m2);
                }		  
	   }
	   }else{
	      return;
	...
	//鼠标右键结束事件
	hander_m2.setInputAction(function() {
       if(drawFlag_m2=true){
	       var areaNum=0;
			for(var i=0;i<polygonPath_m2.length-2;i++){
			var aSide=calculateDis(polygonPath_m2[0],polygonPath_m2[i+1]);
			var bSide=calculateDis(polygonPath_m2[0],polygonPath_m2[i+2]);
			var cSide=calculateDis(polygonPath_m2[i+1],polygonPath_m2[i+2]);
		...
		var cartographicFinal=ellipsoid.cartesianToCartographic(polygonPath_m2[polygonPath_m2.length-1]);
	...
	//生成面的构造函数
	function CreatPolygon_m1(positions){
	...
	CreatPolygon_m1.prototype.init=function(){
	    var that=this;
