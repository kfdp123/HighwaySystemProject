...
var CreateRectangle = (function() {
        ...
        _.prototype._init = function() {
            ...
            var positionCBP1 = function() {
				...
				/* num=Cesium.Rectangle.fromCartographicArray(that.path[0],that.path[1],that.path[2],that.path[3],that.path[4],that.path[5]); */
...
imgsArray.push(imgTSZB_red(latitudeValue,longitudeValue,2,color1_Y,color1_N,calculateDis_toshi(tongshiArray[0],tongshiArray[1])/111000,50,calLatValue(tongshiArray[0],tongshiArray[1])));
imgsArray.push(imgTSZB_green(latitudeValue,longitudeValue,2,color1_Y,color1_N,calculateDis_toshi(tongshiArray[0],tongshiArray[1])/111000,50,calLatValue(tongshiArray[0],tongshiArray[1])));
	...
	handler6.setInputAction(function() {
	 var cartographic=viewer.scene.globe.ellipsoid.cartesianToCartographic(tongshiArray[0]);
		...
		imgsArray.push(imgTSZB_red(latitudeTS,longitudeTS,2,color1_Y,color1_N,calculateDis_toshi(tongshiArray[0],tongshiArray[1])/111000,50,calLatValue(tongshiArray[0],tongshiArray[1])));
		imgsArray.push(imgTSZB_green(latitudeTS,longitudeTS,2,color1_Y,color1_N,calculateDis_toshi(tongshiArray[0],tongshiArray[1])/111000,50,calLatValue(tongshiArray[0],tongshiArray[1])));
					viewer.entities.removeById('ts'+ff);
...
function tongshiAnalyst_Cancel(){
	...
	viewer.scene.imageryLayers.remove(imagelayers);
	//viewer.scene.imageryLayers.remove(image);
}
//坡度函数
...
var image_type = layers.addImageryProvider(new Cesium.SingleTileImageryProvider({
		url :'http://localhost:14282//water.aspx?lat1=30&lon1=110&lat2=31&lon2=111&h=580', 
...
function temerate(){
	...
	if (poduArray[3]>poduArray[1]&&poduArray[2]>poduArray[0]){
			URL ='http://localhost:14282//water.aspx?lat1='+ poduArray[1] +'&lon1='+ poduArray[0]+'&lat2='+poduArray[3]+'&lon2='+poduArray[2]+'&h='+tt1;
			poduArray1=[poduArray[1],poduArray[0], poduArray[3],poduArray[2]];
		}else if(poduArray[3]>poduArray[1]&&poduArray[0]>poduArray[2]){
			URL ='http://localhost:14282//water.aspx?lat1='+ poduArray[1] +'&lon1='+ poduArray[2]+'&lat2='+poduArray[3]+'&lon2='+poduArray[0]+'&h='+tt1;
			poduArray1=[poduArray[1],poduArray[2], poduArray[3],poduArray[0]];
		}else if (poduArray[1]>poduArray[3]&&poduArray[2]>poduArray[0]){
			URL ='http://localhost:14282//water.aspx?lat1='+ poduArray[3] +'&lon1='+ poduArray[0]+'&lat2='+poduArray[1]+'&lon2='+poduArray[2]+'&h='+tt1;
			poduArray1=[poduArray[3],poduArray[0], poduArray[1],poduArray[2]];
		}else if (poduArray[1]>poduArray[3]&&oduArray[0]>poduArray[2]){
			URL ='http://localhost:14282//water.aspx?lat1='+ poduArray[3] +'&lon1='+ poduArray[2]+'&lat2='+poduArray[1]+'&lon2='+poduArray[0]+'&h='+tt1;
...
var YM_POLYGON =(function() {
        ...
        _.prototype._init = function() {
            ...
            this.options.position = new Cesium.CallbackProperty(positionCBP1, false);
			this.options.polygon.semiMinorAxis=new Cesium.CallbackProperty(positionCBP2, false);
			this.options.polygon.semiMajorAxis=new Cesium.CallbackProperty(positionCBP2, false);
            viewer.entities.add(this.options);
        };
        return _;
    })();
...
function imgTSZB_green(arc1,arc2,arc3,arc4,arc5,arc6,arc7,arc8)
{	

    var image_type = layers.addImageryProvider(new Cesium.SingleTileImageryProvider({
		url :'http://localhost:14283/slope_green.aspx?lat='+arc1+'&lon='+arc2+'&len='+arc6+'&c1='+arc4+'&c2='+arc5+'&h='+arc7+'&num='+arc8, 
		rectangle : Cesium.Rectangle.fromDegrees(arc2-arc6, arc1-arc6, arc2+arc6, arc1+arc6)
		}));
	image_type.alpha=0.7;
	return image_type;

}

function imgTSZB_red(arc1,arc2,arc3,arc4,arc5,arc6,arc7,arc8)
{	

    var image_type = layers.addImageryProvider(new Cesium.SingleTileImageryProvider({
		url :'http://localhost:14283/slope_red.aspx?lat='+arc1+'&lon='+arc2+'&len='+arc6+'&c1='+arc4+'&c2='+arc5+'&h='+arc7+'&num='+arc8, 
		rectangle : Cesium.Rectangle.fromDegrees(arc2-arc6, arc1-arc6, arc2+arc6, arc1+arc6)
		}));
	image_type.alpha=0.1;
	return image_type;
